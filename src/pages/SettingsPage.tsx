...
function SettingsRow({ icon, title, subtitle, onClick }: SettingsRowProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex min-h-[76px] w-full items-center gap-3 px-3.5 py-2.5 text-left transition hover:bg-slate-50/80 focus:outline-none focus-visible:ring-2 focus-visible:ring-sage/30"
    >
      <span className="flex h-[43px] w-[43px] shrink-0 items-center justify-center rounded-[13px] bg-sage/10 text-[20px] text-moss">
        {icon}
      </span>
      ...
      <span className="flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[10px] bg-slate-100 text-sm font-bold text-slate-400 transition group-hover:bg-sage/10 group-hover:text-moss">
        ›
      </span>
    </button>
  )
...
function SettingsGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-5">
      ...
      <div className="overflow-hidden rounded-[19px] bg-white shadow-sm ring-1 ring-black/5">
        {children}
      </div>
    </section>
  )
    ...
    <div className="mx-auto w-full max-w-3xl px-1 pb-8 sm:px-2 lg:max-w-4xl">
      ...
      <section className="mb-6 rounded-[24px] bg-white px-[18px] py-6 text-center shadow-sm ring-1 ring-black/5 sm:mb-7">
        <div className="mx-auto mb-3 flex h-24 w-24 items-center justify-center rounded-full border-2 border-moss/20 bg-moss/10 p-1.5">
          {profile.picture ? (
            <img src={profile.picture} alt="Profile" className="h-full w-full rounded-full object-cover" />
          ) : (
            <div className="flex h-full w-full items-center justify-center rounded-full bg-moss text-3xl font-black text-white">
              {profile.initials}
            </div>
          )}
        </div>
      ...
      <SettingsGroup title="PREFERENCES">
        <SettingsRow
          ...
          onClick={() => comingSoon('Appearance settings')}
        />
        ...
        <SettingsRow
          ...
          onClick={() => comingSoon('Recipe creation settings')}
        />
        <div className="ml-[69px] h-px bg-black/5" />
        <SettingsRow
          icon="◉"
          ...
          onClick={() => comingSoon('Interactive cooking settings')}
        />
      </SettingsGroup>
      ...
      <SettingsGroup title="ACCOUNT">
        <SettingsRow
          ...
          onClick={() => comingSoon('Account information')}
        />
        <div className="ml-[69px] h-px bg-black/5" />
        <SettingsRow
          icon="◇"
          ...
          onClick={() => comingSoon('Security settings')}
        />
        <div className="ml-[69px] h-px bg-black/5" />
        <SettingsRow
          icon="ⓘ"
          title="About"
          subtitle="MUVETH Kitchen · v0.2.0"
      ...
      <SettingsGroup title="QUICK PREFERENCES">
        <label className="flex min-h-[76px] cursor-pointer items-center justify-between gap-4 px-3.5 py-2.5">
          <div className="min-w-0">
            <p className="text-[15px] font-extrabold text-ink">Cooking notifications</p>
            <p className="mt-0.5 text-[12px] leading-[17px] text-slate-500">Receive useful kitchen reminders.</p>
          </div>
          <input
            type="checkbox"
            checked={notifications}
            onChange={(event) => setNotifications(event.target.checked)}
            className="h-5 w-5 shrink-0 accent-[#3d513f]"
          />
        </label>
        <div className="ml-[69px] h-px bg-black/5" />
        <label className="flex min-h-[76px] cursor-pointer items-center justify-between gap-4 px-3.5 py-2.5">
          <div className="min-w-0">
            <p className="text-[15px] font-extrabold text-ink">Compact recipe cards</p>
            <p className="mt-0.5 text-[12px] leading-[17px] text-slate-500">Show more recipes at once.</p>
          </div>
          <input
            type="checkbox"
            checked={compact}
      ...
      <div className="mb-6 flex justify-end">
        <button
          type="button"
          onClick={save}
          className="rounded-full bg-ink px-6 py-3 text-sm font-semibold text-white transition hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-ink/30"
        >
          Save settings
        </button>
      </div>
      ...
      <button
        ...
        className="mb-7 w-full rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-left text-sm font-semibold text-red-600 transition hover:bg-red-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-200"
      >
        Sign out of MUVETH Kitchen
      </button>

      <p className="text-center text-[9.5px] font-black tracking-[.17em] text-slate-400">
        MUVETH · HEALTH
      </p>
    </div>
  )
